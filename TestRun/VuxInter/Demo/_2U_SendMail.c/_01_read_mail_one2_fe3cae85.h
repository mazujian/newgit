...
//VuxInter/Demo/_2U_SendMail.c/_01_read_mail_one2_fe3cae85.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/_01_read_mail_one2(const char*,size_t))
#ifdef _VUDEBUG
static Mail* _01_read_mail_one2_vpls_(const char* p_str,size_t size){ VuxDbgBrkP()
{
    size_t i;
    char *p_src, *p_des; 
    BOOL find_at = 0;  //是否有@
...
#else
static Mail* _01_read_mail_one2_vpls_(const char* p_str,size_t size){VUX_VARIABLE(2) VUX_VARIABLE(5)  __VUFNINFO__("_01_read_mail_one2","Mail*",0);VuxDbp(0,0,35);
{
...
VUXRLNFVB(4,4,5);    BOOL find_dot = 0; VUXLV("BOOL",find_dot,5); //@后是否有.
    ...
    /*[学习提示：底层输入]    添加：在左边函数代码窗口，双击函数名，如malloc。    填值：不填值则调用实际代码，填值格式与其他数据相同。    举例：malloc() return在用例1填NULL，其他用例不填，	      则用例1返回NULL，其他用例正常申请内存。	*/
VUXRLNFVB(5,5,6);    Mail* p_mail = (Mail*)malloc(sizeof(Mail)); VUXLV("Mail*",p_mail,6);
...
static Mail* _01_read_mail_one2_vpls(const char* p_str,size_t size)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return _01_read_mail_one2(p_str,size);}
	else if(VuxIsRepeatRun()) {return _01_read_mail_one2_vpls_(p_str,size);}

#ifdef _VUDEBUG
    {Mail* vuxRet =  VuxIsDebugPlug() ? _01_read_mail_one2_vpls_(p_str,size) : _01_read_mail_one2(p_str,size);
#else
    {Mail* vuxRet = _01_read_mail_one2_vpls_(p_str,size);
#endif
    TEST_ASSERT_RETURN("Mail*", &vuxRet);
    VuxResetRunTimes();
    return vuxRet;}
}
//$$VUNIT(END,TESTFUNCPLUG/_01_read_mail_one2(const char*,size_t))
