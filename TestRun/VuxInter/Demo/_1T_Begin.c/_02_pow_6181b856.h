...
//VuxInter/Demo/_1T_Begin.c/_02_pow_6181b856.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/_02_pow(unsigned int,unsigned int))
#ifdef _VUDEBUG
static unsigned int _02_pow_vpls_(unsigned int g,unsigned int e){ VuxDbgBrkP()
{
...
#else
static unsigned int _02_pow_vpls_(unsigned int g,unsigned int e){VUX_VARIABLE(2)  __VUFNINFO__("_02_pow","unsigned int",0);VuxDbp(0,0,15);
{
    /*[学习提示：VU4基本操作]    建立更多用例：选中一列，并点击"添加用例"	切换用例输出：右键菜单可切换用例；	              点击失败的测试，自动切换到对应的用例。	观察程序行为：对比当前用例的输入输出以及所执行的代码。	              黑色代码为当前用例所执行的代码。    找出错误原因：观察程序行为，可发现由于result初始值为0，	              导致for循环的计算结果恒为0。	白盒覆盖标记：[T/F]未覆盖的条件真/假值,[M]未覆盖MCDC；	              <T/F>未覆盖的判定真/假值；				  红色的分支或路径未覆盖。	完成白盒覆盖：选中未覆盖单位，右键菜单选择"用例设计" 。	              关于白盒覆盖及用例设计器的使用，请看_6T_WhiteBox.cpp中的_01_WhileBox()。	标记不可覆盖：for循环的第2分支不可覆盖，可标记为不可覆盖。方法：	              点击未覆盖单位，右键菜单选择"删除逻辑单位"。    */
VUXRBV(1);VUXRLNFVB(1,1,1);    unsigned int result = 0; VUXLV("unsigned int",result,1);
...
static unsigned int _02_pow_vpls(unsigned int g,unsigned int e)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return _02_pow(g,e);}
	else if(VuxIsRepeatRun()) {return _02_pow_vpls_(g,e);}

#ifdef _VUDEBUG
    {unsigned int vuxRet =  VuxIsDebugPlug() ? _02_pow_vpls_(g,e) : _02_pow(g,e);
#else
    {unsigned int vuxRet = _02_pow_vpls_(g,e);
#endif
    TEST_ASSERT_RETURN("unsigned int", &vuxRet);
    VuxResetRunTimes();
    return vuxRet;}
}
//$$VUNIT(END,TESTFUNCPLUG/_02_pow(unsigned int,unsigned int))
