...
//VuxInter/Demo/_2T_SendMail.c/_04_read_tail_84f76416.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/_04_read_tail(char*,size_t,char**))
#ifdef _VUDEBUG
static size_t _04_read_tail_vpls_(char* p_str,size_t buf_size,char** pp_tail){ VuxDbgBrkP()
{
    size_t i;
    size_t count = 0;
    if(buf_size == 0)
...
#else
static size_t _04_read_tail_vpls_(char* p_str,size_t buf_size,char** pp_tail){VUX_VARIABLE(2)  __VUFNINFO__("_04_read_tail","size_t",0);VuxDbp(0,0,18);
{
...
static size_t _04_read_tail_vpls(char* p_str,size_t buf_size,char** pp_tail)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return _04_read_tail(p_str,buf_size,pp_tail);}
	else if(VuxIsRepeatRun()) {return _04_read_tail_vpls_(p_str,buf_size,pp_tail);}

#ifdef _VUDEBUG
    {size_t vuxRet =  VuxIsDebugPlug() ? _04_read_tail_vpls_(p_str,buf_size,pp_tail) : _04_read_tail(p_str,buf_size,pp_tail);
#else
    {size_t vuxRet = _04_read_tail_vpls_(p_str,buf_size,pp_tail);
#endif
    TEST_ASSERT_RETURN("size_t", &vuxRet);
    VuxResetRunTimes();
    return vuxRet;}
}
//$$VUNIT(END,TESTFUNCPLUG/_04_read_tail(char*,size_t,char**))
