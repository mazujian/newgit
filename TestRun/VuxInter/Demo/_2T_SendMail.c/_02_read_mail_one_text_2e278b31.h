...
//VuxInter/Demo/_2T_SendMail.c/_02_read_mail_one_text_2e278b31.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/_02_read_mail_one_text(char**,char*,size_t))
#ifdef _VUDEBUG
static BOOL _02_read_mail_one_text_vpls_(char** pp_str,char* p_mail_text,size_t size){ VuxDbgBrkP()
{
    size_t i = 0;
    size_t count = 0;
    char* p_str = 0;
...
#else
static BOOL _02_read_mail_one_text_vpls_(char** pp_str,char* p_mail_text,size_t size){VUX_VARIABLE(2)  __VUFNINFO__("_02_read_mail_one_text","BOOL",0);VuxDbp(0,0,21);
{
...
static BOOL _02_read_mail_one_text_vpls(char** pp_str,char* p_mail_text,size_t size)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return _02_read_mail_one_text(pp_str,p_mail_text,size);}
	else if(VuxIsRepeatRun()) {return _02_read_mail_one_text_vpls_(pp_str,p_mail_text,size);}

#ifdef _VUDEBUG
    {BOOL vuxRet =  VuxIsDebugPlug() ? _02_read_mail_one_text_vpls_(pp_str,p_mail_text,size) : _02_read_mail_one_text(pp_str,p_mail_text,size);
#else
    {BOOL vuxRet = _02_read_mail_one_text_vpls_(pp_str,p_mail_text,size);
#endif
    TEST_ASSERT_RETURN("BOOL", &vuxRet);
    VuxResetRunTimes();
    return vuxRet;}
}
//$$VUNIT(END,TESTFUNCPLUG/_02_read_mail_one_text(char**,char*,size_t))
