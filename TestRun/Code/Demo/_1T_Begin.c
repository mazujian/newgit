...
{
 VUXRSVB(1,1);    int count = 0; VUXLV("int",count,1); //左边空格的数量
...
{void* vmckrt=0; VMCKDS vmckds[1]={{1,"char*","str",(const void*)&str}}; if(VuxMck("_03_strtrml_2",&vmckrt,vmckds,1,"",940,9,1)) return *(char**)vmckrt;{__VUFNINFO__("_03_strtrml_2","char*",940); VuxDbgBrkO() 
{
 ...
 VUXRS(5);        count++;
    ...
    /*[学习提示：局部输出]    添加：在左边函数代码窗口，点击输出位置，右键菜单选择"局部输出"。    填值：如要判断输出，则填值。无论是否判断，均会打印实际结果。    如果只是编码过程中打印数据，不需要判断，可以直接在代码中添加：	//>>type, name，如//>>int, count	*/    
...
{
 VUXRSVB(1,1);    char* ptr = str; VUXLV("char*",ptr,1); //保存初始指针
 VUXRSVB(2,2);    int count = 0; VUXLV("int",count,2); //左边空格的数量
